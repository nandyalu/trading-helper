...
The transaction log (bot/storage.py's ``state["positions"]``) is the only persisted
...
def get_current_price(ticker: str) -> float | None:
    ...
    Prefers Webull's real-time snapshot when configured (bot/quotes.py) and
    falls back to yfinance's delayed close."""
    from bot.quotes import get_realtime_price  # lazy: keeps positions import-light
