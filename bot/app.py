...
the process now; bot/main.py just runs it via uvicorn. Replaces
bot/api/app.py and the old discord.py-owns-the-loop model entirely.
"""
import logging
...
from bot.discord_client import start_discord, stop_discord
from bot.scheduler import register_jobs, scheduler

log = logging.getLogger("trading-bot.app")

# bot/app.py -> bot -> /app (repo root in the container, repo root locally)
