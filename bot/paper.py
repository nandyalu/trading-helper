...
whole open position (no shorting). Reuses bot/positions.py's FIFO math.
...
from bot.portfolio import open_book_vs_spy
from bot.positions import Position, compute_position, get_current_price, signed_dollars
from bot.signals import BUYISH_DECISIONS, SELLISH_DECISIONS
...
def _performance_lines(open_positions: dict) -> list[str]:
    ...
    comparison (reused from bot/portfolio.py) on the open paper book."""
    lines: list[str] = []
