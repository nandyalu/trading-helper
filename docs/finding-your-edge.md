...
- A second limit then caps the position at 20% of equity. Risk sizing alone does not bound concentration: a stock with a small ATR produces a large share count from a small risk budget. On a $5,000 account at 1% risk, a $50 stock with a $0.50 ATR sizes to 50 shares — $2,500, half the account — from a $50 risk budget. The cap cuts that to $1,000.
- `max_positions` (5 by default) warns you when the book is already full. On a small account you cannot fund every signal, and a 1 to 2 week horizon produces far more of them than the money allows.
...
Paper buys use exactly this sizing once equity is set, which is what makes the paper equity curve evidence about your real account rather than about a hypothetical larger one.
Without equity configured they fall back to a flat notional.
